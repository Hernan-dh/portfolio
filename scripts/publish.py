...
def gemini_response_text(response: dict[str, object]) -> str:
    ...
    if not texts:
        raise ValueError(f"Gemini returned no text (finish reason: {candidate.get('finishReason', 'unknown')}).")
    return "\n".join(texts)
...
if arguments.title and arguments.description:
    ...
    proposed_title, proposed_description = generate_proposal(paths)
