const translations = {
  en: {
    operationsKind: 'AI automation · 2026', operationsDescription: 'An auditable operations-triage application that turns unstructured access, billing, and technical-support requests into documented cases.', operationsDetails: 'A Node.js API sends every request through a required n8n workflow, where Gemini classification is schema-validated before deterministic policy retrieves the procedure and enforces human review. PostgreSQL persists each case and its approval or rejection lifecycle; if n8n is unavailable, the application fails closed.', operationsRole: 'Design, development, and documentation', operationsStatus: 'Live project', operationsScreenshotAlt: 'AI Operations Desk analyzing a repository-access request', operationsScreenshotCaption: 'Case analysis interface', operationsDemoLabel: 'Open project ↗', operationsDemoTitle: 'Open AI Operations Desk', operationsRepoLabel: 'View repository ↗', operationsRepoTitle: 'AI Operations Desk source repository',
  ...
  es: {
    operationsKind: 'Automatización con IA · 2026', operationsDescription: 'Aplicación auditable de triage operativo que convierte solicitudes desestructuradas de acceso, facturación y soporte técnico en casos documentados.', operationsDetails: 'Una API Node.js envía cada solicitud por un workflow obligatorio de n8n, donde la clasificación de Gemini se valida contra un esquema antes de que una política determinista recupere el procedimiento y aplique la revisión humana. PostgreSQL conserva cada caso y su ciclo de aprobación o rechazo; si n8n no está disponible, la aplicación falla de forma segura.', operationsRole: 'Diseño, desarrollo y documentación', operationsStatus: 'Proyecto en línea', operationsScreenshotAlt: 'AI Operations Desk analizando una solicitud de acceso a repositorio', operationsScreenshotCaption: 'Interfaz de análisis de caso', operationsDemoLabel: 'Abrir proyecto ↗', operationsDemoTitle: 'Abrir AI Operations Desk', operationsRepoLabel: 'Ver repositorio ↗', operationsRepoTitle: 'Repositorio fuente de AI Operations Desk',
...
Object.assign(translations.en, {
  ...
  profileTwo: 'My current work focuses on automation, workflow design, AI, IoT monitoring, Linux and Docker infrastructure, operational data, and reliable human-in-the-loop systems.',
...
Object.assign(translations.es, {
  ...
  profileTwo: 'Mi trabajo actual se concentra en automatización, diseño de workflows, IA, monitoreo IoT, infraestructura Linux y Docker, datos operativos y sistemas confiables con supervisión humana.',
...
Object.assign(translations.en, {
  ...
  greenhouseDescription: 'An IoT monitoring and technical-operations system for a live growing environment. It combines Home Assistant, Tuya sensors, pump monitoring, MQTT, dashboards, historical data, scheduled reports, Telegram alerts, and a responsive web interface to make environmental conditions and operational incidents observable.',
  greenhouseDetails: 'Home Assistant coordinates physical automations, while Python, FastAPI, SQLite, and Docker support telemetry, authenticated web operations, reporting, and alerting. A watchdog recovers stalled sensor integration; Telegram bots and a sandboxed OpenClaw agent use controlled MCP tools and an isolated analytical replica for read-only operational queries, charts, exports, and human-reviewed recommendations.',
...
Object.assign(translations.es, {
  ...
  greenhouseDescription: 'Sistema de monitoreo IoT y operaciones técnicas para un entorno de cultivo en funcionamiento. Combina Home Assistant, sensores Tuya, monitoreo de bomba, MQTT, dashboards, datos históricos, reportes programados, alertas por Telegram y una interfaz web adaptable para hacer observables las condiciones ambientales y las incidencias operativas.',
  greenhouseDetails: 'Home Assistant coordina las automatizaciones físicas, mientras Python, FastAPI, SQLite y Docker respaldan telemetría, operaciones web autenticadas, reportes y alertas. Un watchdog recupera integraciones de sensores detenidas; bots de Telegram y un agente OpenClaw aislado usan herramientas MCP controladas y una réplica analítica aislada para consultas operativas de solo lectura, gráficos, exportaciones y recomendaciones sujetas a revisión humana.',
...
Object.assign(translations.en, {
  operationsDescription: 'An auditable operations-triage application that turns unstructured access, billing, and technical-support requests into documented cases.',
  operationsDetails: 'A Node.js API sends every request through a required n8n workflow, where Gemini classification is schema-validated before deterministic policy retrieves the procedure and enforces human review. PostgreSQL persists each case and its approval or rejection lifecycle; if n8n is unavailable, the application fails closed.',
  ...
  debateDetails: 'The Gradio 6 interface and YAML tasks run sequentially; the Debate Agent can use bounded Serper or DDGS evidence, then the Judge Agent receives the completed arguments. Per-call Gemini, Groq, and OpenRouter fallback preserves completed task context when a provider fails. Completed debates are downloadable as Markdown, DOCX, or PDF; follow-up chat stays grounded in the debate, while `/new-motion` starts a new one.',
  ...
  financialDetails: 'The Gradio 6 and CLI interfaces route a company through Serper-backed research, sequential YAML tasks, and analysis. Python, CrewAI, Gemini, Groq, and OpenRouter fallback preserve completed research when a model provider fails. Completed reports are downloadable as Markdown, DOCX, or PDF; follow-up chat stays grounded in the report, while `/new-report` starts new company research.',
  ...
  stockDetails: 'The Gradio 6 and CLI interfaces take a sector, use bounded Serper searches, and pass Pydantic-validated company lists through sequential YAML tasks to a final candidate selection. Gemini, Groq, and OpenRouter fallback detects malformed or repetitive output without restarting completed work. Completed reports are downloadable as Markdown, DOCX, or PDF; follow-up chat stays grounded in the report, while `/new-report` starts a new sector analysis.'
});

Object.assign(translations.es, {
  operationsDescription: 'Aplicación auditable de triage operativo que convierte solicitudes desestructuradas de acceso, facturación y soporte técnico en casos documentados.',
  operationsDetails: 'Una API Node.js envía cada solicitud por un workflow obligatorio de n8n, donde la clasificación de Gemini se valida contra un esquema antes de que una política determinista recupere el procedimiento y aplique la revisión humana. PostgreSQL conserva cada caso y su ciclo de aprobación o rechazo; si n8n no está disponible, la aplicación falla de forma segura.',
  ...
  financialDetails: 'Las interfaces Gradio 6 y CLI procesan una empresa mediante investigación respaldada por Serper, tareas YAML secuenciales y análisis. Python, CrewAI y el fallback entre Gemini, Groq y OpenRouter preservan la investigación completada si falla un proveedor de modelos. Los informes terminados se descargan como Markdown, DOCX o PDF; la conversación posterior queda fundamentada en el informe y `/new-report` inicia una investigación nueva de empresa.',
